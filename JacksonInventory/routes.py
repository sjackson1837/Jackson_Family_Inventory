...
def use_item():
    ...
    if existing_item:
        ...
        product_name = existing_item.productname
        updated_qty = existing_item.qty
        flash(f'Product: {product_name} now has {updated_qty}', 'success')
        # Redirect to the /use_item webpage
        return jsonify({'redirect_url': url_for('use_item')})
    else:
        # Flash an error message
        #flash('Barcode Not Found', 'error')
        # Return a JSON response with the error message
        return jsonify({'barcode_not_found': True, 'redirect_url': url_for('use_item')})
